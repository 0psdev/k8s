...
sudo kubeadm init --kubernetes-version=1.33.5 --cri-socket=unix:///var/run/crio/crio.sock --pod-network-cidr=x.x.x.x --service-cidr=y.y.y.y --control-plane-endpoint=name 

*** x.x.x.x=network-ip-pod y.y.y.y=network-ip-service name=endpoint-access ***
